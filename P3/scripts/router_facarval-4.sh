...
/bin/sh -c "/usr/lib/frr/docker-start & sleep 2 && ip addr add 10.1.1.10/30 dev eth0 && ip addr add 1.1.1.4/32 dev lo && ip link set eth0 up && ip link add vxlan10 type vxlan id 10 dstport 4789 local 1.1.1.4 nolearning && ip link set vxlan10 up && ip link add br0 type bridge && ip link set br0 up && ip link set vxlan10 master br0 && ip link set eth1 master br0 && while true; do sleep 1000; done"